...
confuse <- table(BigChungus[,9],
                 classPredictions)
confuse

# Estimate the Misclassification Rate
numCorrect <- sum(diag(confuse))
numEstimated <- sum(confuse)
...
library(nnet)
lr <- multinom(isDiabetic ~., data = BigChungus, trace=FALSE, maxit = 10000)
...
Confusion <- table(BigChungus[,9],class)
Confusion
right <- 0
total <- 0
...
for (i in 1:n) {
  lr <- multinom(isDiabetic~., data = BigChungus[-i,], trace=FALSE, maxit = 10000)
  coe <- summary(lr)$coefficients
...
SUMMARY. Again, comments in the code reveal specific tasks. 
```{r, eval=FALSE}
# Load the Classifcation Library
library(class)

# Make log scale
kgrid <- 10^seq(.5,2.5,length.out = 20)
kgrid <- floor(kgrid)
MCR <- rep(0,20)

folds <- sample(c(1:nrow(BigChungus)),replace = FALSE)
foldind <- seq(1,768,length.out = 11)
foldind <- floor(foldind)

# Make test-train splits
for(i in 1:length(kgrid)){
MCRS <- rep(0,10) 
  for(j in 1:10){
    testind <- foldind[j]:foldind[j+1]
  test <- BigChungus[testind,]
  train <- BigChungus[-testind,]
    fit <- knn(train, test, cl = train[,9], k=kgrid[i])
...
# New fit of entire Dataset with k =21

FinalFit = knn(BigChungus, BigChungus, cl = BigChungus[,9], k = KBest)
