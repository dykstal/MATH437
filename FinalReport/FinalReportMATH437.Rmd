...
Since all of the covariates are numeric, we can calculate distances between $\boldsymbol{x}$ and other covariate vectors. Thus, the conditions are satisfied to perform knn regression. Additionally, we made sure to pick the best value for k, as shown in section 4.2
...
```
Because of the potenital collinearity between the BMI and the FINISH ME FUCK.
```{r}
# Bring in the NNET Library for Logistic Regression
library(nnet)
