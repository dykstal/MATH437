...
\item (Schreiber-Gregory) Assumption of appropriate outcome structure. This means that the outcome's format should align with the datas. Here, our outcome is binary (has Type II Diabetes or does not), and our data has the same format, therefore this assumption holds in this experiement.

 
\item (Schreiber-Gregory) Assumption of observation independence. Here, we assume that the data is independent - not from repeated measurements or matched data. This is met in our data set because each observation is unique.   


\item (Schreiber-Gregory)  Assumption of the absence of multicollinearity. This means that our independent variables should not be ``too highly correlated" with one another. (CHECK THIS BELOW)


\item (Schreiber-Gregory) Assumption of Large sample size. This requires a sufficiently large sample size to represent the group we wish to make inference on. Because we have 768 observations, this assumption is met satisfactorily. 


\item (Schreiber-Gregory) Assumption of linearity of independent variables and log odds. This assumes the independent variable is linearly related to the logodds. (CHECK THIS BELOW)


\item (Navidi) Assume each outcome is a benoulli trial with probability $p$


\item (Navidi) $p = \frac{exp(\beta_0 + \beta_1x_1 + ... + \beta_px_p)}{1 + exp(\beta_0 + \beta_1x_1 + ... + \beta_px_p)} = \frac{exp(\beta^Tx)}{1 + exp(\beta^Tx)}$
...
\end{enumerate}
Four our purposes, DESCRIBE HOW WE MEET. USE EUCLIDEAN DISTANCE. GO PACK GO.
