...
\item Different covariance for each of the response classes. That is, for ex – $\sigma_{k1}$, $\sigma_{k2}$, $\sigma_{k3}$ for response class $k1$, $k2$, $k3$ etc.
...
Logstic Regression requires the following assumptions for validity of the model:
\begin{enumerate}

\item (Schreiber-Gregory) Appropriate response structure, meaning that the response's format aligns with the data. Here, our response is binary (has Type II Diabetes or does not) and our data has the same format. Therefore, this assumption holds in this experiement.
 
\item (Schreiber-Gregory) Observation independence, meaning the observations in the data are independent. This means that observations are not the result of repeated measurements or matched data. This is met in our data set because each observation is unique (each person sampled is distinct).

\item (Schreiber-Gregory)  Absence of multicollinearity, meaning our independent variables should not be "too highly correlated" with one another. We will check this assumption in detail in Section 3.4.

\item (Schreiber-Gregory) Sufficiently large sample size and sufficient representation for each group. By convention, this is usually considered to be roughly 30 in a classification problem. We have 768 total observations, 500 of which belong to the non-diabetic group and 268 of which belong to the diabetic group. Clearly, this assumption is satisfactorily met. 

\item (Schreiber-Gregory, Navidi) Linearity of independent variables and log odds. This assumes the independent variable is linearly related to the logodds. Luckily, the logistic regression model defines the maximum likelihood estimator as
\begin{equation*}
  p = \frac{\exp(\beta_0 + \beta_1x_1 + \cdots + \beta_px_p)}{1 + \exp(\beta_0 + \beta_1x_1 + \cdots + \beta_px_p)} = \frac{\exp(\boldsymbol{\beta}^T\boldsymbol{x})}{1 + \exp(\boldsymbol{\beta}^T\boldsymbol{x})},
\end{equation*}
which represents the log odds as a linear combination of the covariates.

\item (Navidi) Assume each outcome is a Bernoulli trial with probability $p$.
...
\end{enumerate}
For our purposes, DESCRIBE HOW WE MEET. USE EUCLIDEAN DISTANCE. GO PACK GO.
