...
Finally, we  check the assumption of the absence of multicollinearity by plotting the covariates against one another. Upon looking at the below plots, we see no noticable trends, including linear trends. One potential exception is the plot of tricep fold thickness versus BMI. In this plot, we concede that these two factors may be related in a linear fashion. But, the linearity is not visually strong. Therefore -- since no two covariates appear to be \emph{strongly} linearly associated with each other -- we conclude that this assumtion is valid. As such, we can conclude that Logistic Regression is reasonably appropriate for classifying the diabetic status of individuals in the Pima Indian population.
