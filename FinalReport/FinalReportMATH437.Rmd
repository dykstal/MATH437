...
\end{tabular}
\end{center}
To find the prior probabilities of each class, we find the sample proportion of observations in the data set belonging to each class. Doing so, we find that roughly 33.271\% of the observations belong to the diabetic class and roughly 66.729\% belong to the non-diabetic class. As such, we estimate the prior probability of the diabetic class as $p_{\text{diabetic}} = 0.33$ and we estimate the prior probability of the non-diabetic class as $p_{\text{diabetic}} = 0.67$.
...
From these plots, we can see that the number of pregnancies ("pregnancyNumber"), the two-hour serum insulin ("serumInsulin"), and the age ("age") are clearly non-normally distributed. The remaining features could reasonably be normally-distributed in the sample. As such, we will re-fit the QDA model, this time omitting the covariates that clearly violate the assumption of normality.
...
\begin{enumerate}
  ...
  \item We continue by choosing the hyper-parameter \texttt{maxit}. The \texttt{maxit} is the maximum number of itterations in the algorithm's search for the minimum of the liklihood. We choose \texttt{maxit} as 10,000 because it finds a balance in computational feasibility and accuracy.  
...
We remove BMI as an explainatory covariate, since tricep fold thickness is a better gauge of body composition and the two covariates could reasonably be collinear. Then, we re-fit our Logistic Regression model to the data without the BMI. Upon doing so, we recalculate the confusion matrix, misclassification rate, and accuracy as before.
