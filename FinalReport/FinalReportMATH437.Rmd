...
\begin{enumerate}
  \item Appropriate response structure, meaning that the response's format aligns with the data. Here, our response is binary (has Type II Diabetes or does not) and our data has the same format. Therefore, this assumption holds in this experiement.
 
  \item Observation independence, meaning the observations in the data are independent. This means that observations are not the result of repeated measurements or matched data. This is met in our data set because each observation is unique (each person sampled is distinct).

  \item Absence of multicollinearity, meaning our independent variables should not be "too highly correlated" with one another. We will check this assumption in detail in Section 3.4.

  \item Sufficiently large sample size and sufficient representation for each group. By convention, this is usually considered to be roughly 30 in a classification problem. We have 768 total observations, 500 of which belong to the non-diabetic group and 268 of which belong to the diabetic group. Clearly, this assumption is satisfactorily met.
  
  \item Each observation is a Bernoulli trial with success probability
\begin{equation*}
...
ELABORATE.
\end{enumerate}
In our case, logistic regression is employed to approach a binary classification problem. The binary classification problem 
which represents the log odds as a linear combination of the covariates.

\item Assume each outcome is a Bernoulli trial with probability $p$.
