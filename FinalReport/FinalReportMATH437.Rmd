...
AIDAN DONT FORGET TO DO THIS YOU FUCK
...
\begin{enumerate}
  \item The response is binary, meaning observations either belong to class zero or class one. In the Pima Indians Diabetes dataset, each observation is either classified as testing positive for Type II diabetes or not, so this assumption holds in our experiement. [1]
 
  \item The observations in the data are independent. In the Pima Indians Diabetes dataset, individual observations are distinct and are drawn from a simple random sample of the Pima Indian population. Thus, there is reasonable evidence that observations are independent in our experiment. [1]

  \item The covariates are not collinear. To check this assumption, we must plot each combination of two covariates against each other to see if there appears to be a strong linear association between them. We will check this assumption in detail in Section 3.4. [1]
...
# Check the assumption that the group-split covariate data is in fact multivariate normal by plotting.
group1 = subset(BigChungus, isDiabetic == 0)
group2 = subset(BigChungus, isDiabetic == 1)
par(mfrow = c(8, 2))
for (i in 1:8) {
  hist(group1[,i], main = NULL, ylab = NULL, xlab = NULL)
  hist(group2[,i], main = NULL, ylab = NULL, xlab = NULL)
}
