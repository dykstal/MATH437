...
\begin{enumerate}
  ...
  \item Next, we fit the $k$NN model to the dataset using leave-one-out cross-validation. That is, for each observation in our data, we predict that point from the model that is trained on every other point.
\end{enumerate}
In both of these cases, we keep track of the number of correctly classified and incorrectly classified observations for each group. By doing so, we construct a confusion matrix showing how frequently individuals are either correctly or incorrectly characterized as diabetic or non-diabetic. As a consequence, we can also compute the experimental accuracy and misclassification rate to estimate the goodness-of-fit for our $k$NN model. The primary difference between using the entire data for training and using leave-one-out cross-validation is that the cross-validation strategy excludes the point we are classifying from the training data. As such, the leave-one-out cross-validation removes a small source of bias from our model. In the context of $k$NN, leave-one-out cross validation ensures that the $k$ nearest neighbors for a given point in the test set do not include that point.
...
In comparing the results of our three different methodologies, we summarize the results as follows: 

\begin{tabular}{c|c|c}\hline
         Method & Accuracy & Cross-Validated Accuracy\\\hline
                       QDA & 77.444\% & 76.503\%\\\hline 
       Logistic Regression & 78.383\% & \bold{78.007\%}\\\hline
                     $k$NN & 79.511\% & 77.067\%\\\hline
\end{tabular}

Obviously the Logistic Regression method yields the best accuracy after cross-validation. 
\newpage
